...
function Note() {
   ...
   * 
   * NOTE: Called on every render. Consider moving to useEffect so it only
   * runs when difficultyLevel changes.
   */
  ...
  /**
   * Generates a random note from the selectionNotesForDifficulty array
   * 
   * @param {number} nums_alphabets - Max index for the note pool (e.g. 6 for 7 notes)
   ...
   * 
   * NOTE: Bug - getRandomNote is passed as a function reference instead of
   * being called. Should be: getRandomNote(numberOfNotes)
   */
  function handleNext(){
   ...
   * The UI consists of:
   * - Page heading
   * - Difficulty selection cards (display only - not yet wired to state)
